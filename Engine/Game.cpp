	...
	Move_Dist(1,3)
{
	...
	{
		block[a].Init( xDist( rng ), yDist ( rng ) , Move_Dist( rng ));
	}
}
...
// New Miner VVVV
	...
	if (wnd.kbd.KeyIsPressed(VK_SHIFT))    // pick up code
	{
		Shift_Switch0 = true;
	}
	else 
   ...
   int temp_y = y;  // makes sure dude doesnt go through blocks by checking start and finish of each move

    miner[New_Miner].Gravity(y); 
	Hit = false;
    bool Col_Check = false;
	for (int a = 0; a < nBlocks; a++)
	{
		block[a].Collision(x, y, temp_y, Col_Check);	
		if (Col_Check)
		{
			Hit = true;
			break;
    ...
    miner[New_Miner].Collision_Check(Hit);
   		 
	miner[New_Miner].Collision(x , y );
		 
	miner[New_Miner].Miner_Health(Dead_Miner_Check);
	
    ore0.Ore_Pos(x, y, Pick_Up);

	ore0.Droped(Dead_Miner_Check);
	
	ore0.Goal(690, 50, 790, 100, Game_Over);
...
void Game::ComposeFrame()
{
	...
	for (int a = 0; a < nBlocks; a++)
	{
		block[a].Draw_Block(gfx);
	}
		if( Dead_Miner >=0)
		{
			for (int a = 0; a <= Dead_Miner; a++)
			{
				miner[a].Miner_Corps(gfx );
			}
		}
        ore0.Draw(gfx);
		miner[New_Miner].Draw(gfx, x, y);
	
    Buttons::Lives_Draw(gfx, Lives);
	Buttons::Button_0_Draw( gfx, x, y);
//	Buttons::Button_1_Draw( gfx, x, y);
//	Buttons::Button_2_Draw( gfx, x, y);
